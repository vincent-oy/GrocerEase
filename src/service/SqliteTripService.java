...
public class SqliteTripService implements TripService {
    ...
    public Trip create(LocalDate date, Integer storeId, int budgetCents, String note) {
        ...
        if (budgetCents < 0)         throw new IllegalArgumentException("budget must be >= 0");
        ...
        try (Connection c = Db.open();
            ...
            t.tripDate = date; t.storeId = storeId; t.budgetCents = budgetCents; t.note = note;
            return t;
    ...
    public TripItem addItem(int tripId, String itemName, String unit, int qty, Integer expectedPriceCents) {
        ...
        try (Connection c = Db.open();
            ...
            ps.setString(3, unit);
            ps.setInt(4, qty);
            if (expectedPriceCents == null) ps.setNull(5, Types.INTEGER); else ps.setInt(5, expectedPriceCents);
            ps.setInt(6, lineTotal);
            ps.executeUpdate();

            TripItem ti = new TripItem();
            try (ResultSet keys = ps.getGeneratedKeys()) { if (keys.next()) ti.id = keys.getInt(1); }
            ti.tripId = tripId; ti.itemName = itemName; ti.unit = unit; ti.plannedQty = qty;
            ti.expectedPriceCents = expectedPriceCents; ti.lineTotalCents = lineTotal;
            return ti;
    ...
    public boolean updateItemQty(int tripItemId, int newQty) {
        ...
        String sql = "UPDATE trip_items SET planned_qty=?, line_total_cents = COALESCE(expected_price_cents,0) * ? WHERE id=?";
        try (Connection c = Db.open();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, newQty);
            ps.setInt(2, newQty);
